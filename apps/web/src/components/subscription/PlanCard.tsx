...
export function PlanCard({
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        <h3 className="text-xl font-bold text-[var(--sv-on-surface)] capitalize">{name}</h3>
        <p className="text-sm text-[var(--sv-on-surface-variant)] mt-1">{description}</p>
      </div>
          ...
          <li key={i} className="flex items-start gap-2">
            <Check
              className={cn(
                ...
                feature.included ? 'text-[var(--sv-primary)]' : 'text-[var(--sv-on-surface-variant)] opacity-40',
              )}
            />
            <span
              className={cn(
