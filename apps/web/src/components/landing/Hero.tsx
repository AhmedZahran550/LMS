...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-[921px] flex items-center overflow-hidden">
      <ShaderBackground />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(249,249,255,1)_100%)] dark:bg-[radial-gradient(circle_at_center,transparent_0%,rgba(21,28,39,1)_100%)]" />
      ...
      <div className="max-w-[1200px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10 w-full">
        ...
        >
          <h1 className="text-5xl md:text-[48px] font-bold text-slate-900 dark:text-slate-100 leading-tight mb-6 tracking-tight">
            {t('Your future')} <span className="text-indigo-600 dark:text-indigo-400 underline decoration-cyan-300 dark:decoration-cyan-600 decoration-4 underline-offset-8">{t('starts here')}</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <button className="bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 px-8 py-4 rounded-xl font-bold text-lg border border-slate-300 dark:border-slate-700 hover:bg-slate-300 dark:hover:bg-slate-700 transition-all active:scale-95 flex items-center gap-2">
              <PlayCircle className="w-5 h-5" />
              {t('How it works?')}
            </button>
          </div>
          
          <div className="mt-12 flex items-center gap-4 text-slate-600 dark:text-slate-400">
            <div className="flex -space-x-3 rtl:space-x-reverse">
              <img
                className="w-10 h-10 rounded-full border-2 border-slate-50 dark:border-slate-900"
            ...
            <span className="text-sm font-medium">
              {t('Join over')} <strong className="text-slate-900 dark:text-slate-100">10,000+</strong> {t('active learners')}
            </span>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="absolute -top-10 -end-10 bg-white/70 dark:bg-slate-900/70 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-white/30 dark:border-white/10 animate-bounce duration-[3000ms]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{t('Certified Diplomas')}</p>
                <p className="font-bold text-sm text-slate-900 dark:text-slate-100">{t('Globally')}</p>
              </div>
            </div>
          </div>
        </motion.div>
