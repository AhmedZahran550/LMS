...
export function VerifyOtpForm({ email }: { email: string }) {
  ...
  if (!email) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-center">{t('Verification')}</CardTitle>
          <CardDescription className="text-center">
            {t('No email provided. Please register first.')}
          </CardDescription>
        </CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardTitle className="text-center">{t('Verify your email')}</CardTitle>
        <CardDescription className="text-center">
      ...
      <CardContent className="space-y-4">
        ...
        {!isExpired ? (
          <div className="flex items-center justify-center gap-2 text-sm text-slate-500">
            ...
            {t('OTP expires in')} <span className="font-semibold text-slate-700">{formatTime(expiry)}</span>
          </div>
        ) : (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium">{t('OTP Code')}</label>
            <Input
              type="text"
              inputMode="numeric"
              maxLength={6}
